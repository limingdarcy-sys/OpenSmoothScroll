...
針對不同軟體的需求（例如瀏覽器希望滑順、編輯器希望精準），您可以為個別程式設定專屬參數：

1. 點擊 **「➕ 瀏覽加入」** 選擇執行檔，或先切換到目標視窗再點擊 **「🔍 偵測程式」**。
2. 點擊 **「✏️ 編輯參數」**，即可跳出專屬設定視窗。
3. 在專屬視窗中調整 Step Size、Animation Time 等參數，這些設定將**覆蓋全域設定**。

### 🚫 排除清單（黑名單）

指定完全不套用平滑捲動的程式（保留原始捲動行為）：
...
您可以使用底部的按鈕將目前的所有設定（包含個別程式參數）匯出為 `config.ini` 檔案，方便在不同電腦間同步或備份。
